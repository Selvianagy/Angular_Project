...
export class MobileComponent implements OnInit {
  ...
  constructor(private productService: ProductServiceService, private mobileService: MobileServiceService) { }
  ...
  BrandFilter() {
    this.filteredMobilesList = this.filteredMobilesList.filter(mobile => { return this.BrandArray.includes(mobile.mainProduct.brandName) })
  }

  PriceFilter() {
    this.PriceArray.forEach(element => {
      if (element === 0) {
        this.filteredMobilesList = this.filteredMobilesList.filter(mobile => mobile.mainProduct.price >= 2000 && mobile.mainProduct.price <= 7000)
      }

      else if (element === 1) {
        this.filteredMobilesList = this.filteredMobilesList.filter(mobile => mobile.mainProduct.price >= 8000 && mobile.mainProduct.price <= 11000)
      }
      else {
        this.filteredMobilesList = this.filteredMobilesList.filter(mobile => mobile.mainProduct.price > 11000)
  ...
  WeightFilter() {
    this.filteredMobilesList = this.filteredMobilesList.filter(mobile => { return this.WeightArray.includes(mobile.weight) })
  }

  CameraFilter() {
    this.filteredMobilesList = this.filteredMobilesList.filter(mobile => { return this.CameraArray.includes(mobile.numberOfCamera) })
  }
  StorageFilter() {
    this.filteredMobilesList = this.filteredMobilesList.filter(mobile => { return this.StoragetArray.includes(mobile.storageCapacity) })
  }
  RamFilter() {
    this.filteredMobilesList = this.filteredMobilesList.filter(mobile => { return this.RamArray.includes(mobile.ram) })
  }

  RateFilter() {
    this.filteredMobilesList = this.filteredMobilesList.filter(mobile => { return this.RateArray.includes(mobile.mainProduct.rates) })
  }
  DiscountFilter() {
    this.filteredMobilesList = this.filteredMobilesList.filter(mobile => { return this.DiscountArray.includes(mobile.mainProduct.discount) })
